...
class User(BaseModel):
    ...
    # Написать скрипт для создания пользователя: периодически обходить выгрузку 1С и добавлять пользователя в таблицу с доступом
    # Написать скрипт для апдейта рали: нужно кажый день обходить новичков и прверять, на прошло ли 3 месяца.
    #                 если роль меняется, то изменить в Seatable и в FSM, и запостить новое меню пользователю.
    ...
    # Написать скрипт, где пользователю создаются отложенные задачи на Пульс-опрос и эти задачи кладутся в таблицу.
    # Пренести сюда is_user_admin из модуля broadcast
